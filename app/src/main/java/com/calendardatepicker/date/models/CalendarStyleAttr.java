...
public class CalendarStyleAttr {
    ...
    public void setAttributes(Context context, AttributeSet attributeSet) {
        if (attributeSet != null) {
            TypedArray ta = context.obtainStyledAttributes(attributeSet, R.styleable.DateMonthView, 0, 0);
            try {
                titleColor = ta.getColor(R.styleable.DateMonthView_title_color, titleColor);
                headerBg = ta.getDrawable(R.styleable.DateMonthView_header_bg);
                weekColor = ta.getColor(R.styleable.DateMonthView_week_color, weekColor);
                rangeStripColor = ta.getColor(R.styleable.DateMonthView_range_color, rangeStripColor);
                selectedDateCircleColor = ta.getColor(R.styleable.DateMonthView_selected_date_circle_color, selectedDateCircleColor);
                shouldEnabledTime = ta.getBoolean(R.styleable.DateMonthView_enable_time_selection, false);
                enabledPastDates = ta.getBoolean(R.styleable.DateMonthView_enable_past_date,false);
                isEditable = ta.getBoolean(R.styleable.DateMonthView_editable,true);

                textSizeTitle = ta.getDimension(R.styleable.DateMonthView_text_size_title, textSizeTitle);
                textSizeWeek = ta.getDimension(R.styleable.DateMonthView_text_size_week, textSizeWeek);
                textSizeDate = ta.getDimension(R.styleable.DateMonthView_text_size_date, textSizeDate);

                selectedDateColor = ta.getColor(R.styleable.DateMonthView_selected_date_color, selectedDateColor);
                defaultDateColor = ta.getColor(R.styleable.DateMonthView_default_date_color, defaultDateColor);
                rangeDateColor = ta.getColor(R.styleable.DateMonthView_range_date_color, rangeDateColor);
                disableDateColor = ta.getColor(R.styleable.DateMonthView_disable_date_color, disableDateColor);
                setWeekOffset(ta.getColor(R.styleable.DateMonthView_week_offset, 0));
